// /api/chat.js — Canned Q&A first, then grounded pages (no guessing). Blocks PDF + “ingredients”.
// JSON + CORS. Friendly tone + emojis.
...
export default async function handler(req, res) {
  ...
  function matchFAQ(q) {
    for (const item of FAQ) if (item.tests.some(rx => rx.test(q))) return item;
    return null;
  }
  ...
  // ---------- 2) GROUNDED PAGES (fallback if no canned match) ----------
  // Blocklists
  const BLOCKED_URL_PATTERNS = [
    /\.pdf($|\?)/i,
    /cdn\.shopify\.com\/.*Ingredient_List\.pdf/i,
  ];
  const BLOCKED_CHUNK_REGEX = /\bingredient(s)?\b|\bingredient list\b|\bfull ingredients?\b/i;
...
// --- Whitelisted QUIT IT pages (edit/expand anytime) ---
  ...
  // Skip blocked URLs entirely
  function isBlockedUrl(url) {
    return BLOCKED_URL_PATTERNS.some(rx => rx.test(url));
  }
  ...
  async function fetchText(url) {
    if (isBlockedUrl(url)) return ""; // never fetch PDFs or blocked assets
  ...
  function chunkText(t, maxLen = 900, overlap = 120) {
    ...
    while (i < t.length) {
      ...
      if (slice && !BLOCKED_CHUNK_REGEX.test(slice)) { // drop any “ingredients” chunks
        chunks.push(slice);
      }
...
"${FRIENDLY_FALLBACK}"
Do NOT enumerate or speculate about specific ingredients. If asked about ingredients, prefer the canned answer when available.
